...
# Banco de dados
```{r}
inicial_data <- read.csv2("bancoSABE_tutorial.csv", header = TRUE)
...
#IMC
sd(inicial_data$imc_novo[inicial_data$morte_5anos == 1], na.rm = TRUE)
sd(inicial_data$imc_novo[inicial_data$morte_5anos == 0], na.rm = TRUE)
...
##mobilidade
#d01b: dificuldade em caminhar varias ruas
...
#d03: dificuldade em se levantar de uma cadeira 
...
#d04: dificuldade em subir varios lances de escada
...
#d06: dificuldade em se curvar, ajoelhar ou agachar
...
#d07: dificuldade para estender os bracos acima dos ombros
...
#d08: dificuldade para puxar/empurrar grandes objetos
...
#d09: dificuldade em levantar ou carregar pesos maiores que 5kg
...
#d10: dificuldade em levantar uma moeda de uma mesa
...
###Estado funcional
##Atividades basicas
#d13a: dificuldade para se vestir
...
#d14a: dificuldade para tomar banho
...
#d16a: dificuldade para deitar ou levantar da cama
...
#d17a: dificuldade para ir ao banheiro
...
#d18a: dificuldade para preparar uma refeicao quente
...
#d19a: dificuldade para cuidar do proprio dinheiro
...
#d25a: dificuldade para tomar seus remedios
...
#d23a: dificuldade para fazer tarefas domesticas leves
...
#d24a: dificuldade para realizar tarefas domesticas mais pesadas
...
mediana_IMC <- median(data_training$imc_novo, na.rm = TRUE)
...
for (i in 1:n) {
     ...
     ifelse (teste[i] == TRUE, data_training$imc_novo_imp[i] <- mediana_IMC,
             data_training$imc_novo_imp[i] <- data_training$imc_novo[i])
}
...
for (i in 1:n) {
     ...
     ifelse (teste[i] == TRUE, data_teste$imc_novo_imp[i] <- mediana_IMC,
             data_teste$imc_novo_imp[i] <- data_teste$imc_novo[i])
}
...
```{r}
Results_RL1
Results_GLMNET1[order(Results_GLMNET1$ROC, decreasing = TRUE), ]
Results_NNET1[order(Results_NNET1$ROC, decreasing = TRUE), ]
Results_BOOST1[order(Results_BOOST1$ROC, decreasing = TRUE), ]
Results_RF1[order(Results_RF1$ROC, decreasing = TRUE), ]
...
AUC1 <- as.data.frame(rbind(AUC_rl1, AUC_glmnet1, AUC_nnet1, AUC_xgb1, AUC_rf1), row.names = FALSE)
names(AUC1)[1] <- "AUC"

IC_AUC1 <- as.data.frame(rbind(IC_AUC_rl1, IC_AUC_glmnet1, IC_AUC_nnet1, IC_AUC_xgb1, IC_AUC_rf1), row.names = FALSE)
...
performance1[order(performance1$AUC, decreasing = TRUE), ]
...
#10% maior risco
df_GLMNET2 <- df_GLMNET[order(df_GLMNET$p_YesGLMNET1, decreasing = TRUE),]
...
#10% maior risco
df_NNET2 <- df_NNET[order(df_NNET$p_YesNNET1, decreasing = TRUE), ]
...
#10% maior risco
df_BOOST2 <- df_BOOST[order(df_BOOST$p_YesBOOST1, decreasing = TRUE), ]
...
#d10% maior risco
df_RF2 <- df_RF[order(df_RF$p_YesRF1, decreasing = TRUE), ]
